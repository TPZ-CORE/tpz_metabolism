...
AddEventHandler("tpz_metabolism:onUsableItemAction", function(index, itemId)
	...
	if itemData.Action.Object then
		LoadObject(itemData.Action.Object)
	end
	...
	if itemData.Action.Animation == "EAT" then
		...
		local prop      = CreateObject(joaat(itemData.Action.Object), coords.x, coords.y, coords.z + 0.2, true, true, false, false, true)
		...
		RemoveEntityProperly(prop, joaat(itemData.Action.Object) )

		ClearPedSecondaryTask(player)

	elseif itemData.Action.Animation == "EAT_BOWL_OR_PLATE" then

		LoadObject("p_spoon01x")

		local prop  = CreateObject(joaat(itemData.Action.Object), coords, true, true, false, false, true)
		local spoon = CreateObject(joaat("p_spoon01x"), coords, true, true, false, false, true)
		...
		TaskItemInteraction_2(player, 599184882, prop, joaat("p_bowl04x_stew_ph_l_hand"), -583731576, 1, 0, -1.0)
		TaskItemInteraction_2(player, 599184882, spoon, joaat("p_spoon01x_ph_r_hand"), -583731576, 1, 0, -1.0)
		...
		RemoveEntityProperly(prop, joaat(itemData.Action.Object) )
		RemoveEntityProperly(spoon, joaat("p_spoon01x") )
	...
	elseif itemData.Action.Animation == "DRINK_LONG_BOTTLE" then
		...
		local entity = CreateObject(joaat(itemData.Action.Object), coords , 0.2, true, true, false, false, true)
		...
		AttachedEntityModel = joaat(itemData.Action.Object)
					...
					for i, prompt in pairs (PromptsList) do
						...
						if PromptHasHoldModeCompleted(prompt.prompt) then
							...
							elseif prompt.type == 'CANCEL' then
								...
								RemoveEntityProperly(entity, joaat(itemData.Action.Object) )
								ClearPedSecondaryTask(player)
	...
	elseif itemData.Action.Animation == "DRINK_MUG" then
		...
		local prop = CreateObject(joaat(itemData.Action.Object), coords, true, true, true)

		TaskItemInteraction_2(player, -1199896558, prop, joaat('p_mugCOFFEE01x_ph_r_hand'), joaat('DRINK_COFFEE_HOLD'), 3, 0, -1.0)
...
OnUsableItemAction = function (index, itemId)
	...
	if itemData.Temperature.Enabled then
		ClientData.ExtraTemperatureType      = itemData.Temperature.Type
		ClientData.ExtraTemperature          = itemData.Temperature.Value
		ClientData.ExtraTemperatureDuration  = itemData.Temperature.Duration
	end
	...
	if metabolismData.Hunger.Value > 0 then
		...
		local returnedValue = GetFixedCount("HUNGER", ClientData.Hunger, metabolismData.Hunger.Type, metabolismData.Hunger.Value)
	...
	if metabolismData.Thirst.Value > 0 then
		...
		local returnedValue = GetFixedCount("THIRST", ClientData.Thirst, metabolismData.Thirst.Type, metabolismData.Thirst.Value)
	...
	if metabolismData.Stress.Value > 0 then
		...
		local returnedValue = GetFixedCount("STRESS", ClientData.Stress, metabolismData.Stress.Type, metabolismData.Stress.Value)
	...
	if itemData.Alcohol.Enabled and itemData.Alcohol.Value > 0 then
		...
		local returnedValue = GetFixedCount("ALCOHOL", ClientData.Alcohol, 'add', itemData.Alcohol.Value)
